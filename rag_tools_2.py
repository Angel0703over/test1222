...
def vector_store(text_chunks: list):
    ...
    try:
        db = FAISS.load_local("space_db", embeddings)
    except:
    ...
    return response_extractor(
        llm.infer(
            ...
            user_prompt=PromptLoader.get_prompt(
                prompt_name='rag/target_space_classification.prompt',
                target_space_threat_description=description
            )
        )
    ).get("result")


from tqdm import tqdm  # 确保导入tqdm库
    ...
    try:
        ...
        with tqdm(total=1, desc="正在计算威胁分数") as pbar:
            prompt = PromptLoader.get_prompt(
                prompt_name='rag/target_space_score.prompt',
                target_space_threat=target_space_threat
            )
...
if __name__ == '__main__':
    # 存储指定的库
    retrieval_data_storage(file_path='./resource/level_2.xlsx', sheet_name='目标空间威胁库')

    PromptLoader.from_paths(['./prompt'])
    # 模拟生成的环境情况
    input_str = """
            敌方集群位于近海区域，与我方目标的距离衰减系数为0.45，相对接近速度达1.8 Ma；该区域敌方目标密度指数为6 k⁻¹・km⁻²，部署集中度达3.7；敌方火力覆盖半径扩展至600 km，对我方目标所在区域的传感器覆盖率90%。
            """
